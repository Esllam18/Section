      ...
      builder: (ctx, state) => Scaffold(
        appBar: AppBar(
          ...
          bottom: TabBar(
            ...
            tabs: [
              Tab(text: isAr ? 'الكل' : 'Global'),
              Tab(text: isAr ? 'كليتي' : 'My Faculty')
            ],
          ),
        ),
        body: switch (state.status) {
          CommunityStatus.loading => ListView.builder(
              itemCount: 4,
              itemBuilder: (_, __) => const Padding(
                  padding: EdgeInsets.fromLTRB(16, 6, 16, 0),
                  child: ShimmerWidget(height: 150, borderRadius: 16))),
          CommunityStatus.error => ErrorStateWidget(
              message: state.error ?? 'Error',
              onRetry: () => ctx.read<CommunityCubit>().load()),
          _ => RefreshIndicator(
              color: AppColors.secondary,
              onRefresh: () => ctx.read<CommunityCubit>().load(),
              child: state.posts.isEmpty
                  ? Center(
                      child: Text(isAr ? 'لا توجد منشورات بعد' : 'No posts yet',
                          style: const TextStyle(
                              fontFamily: 'Cairo',
                  ...
                  : ListView.builder(
                      ...
                      itemCount: state.posts.length +
                          (state.status == CommunityStatus.loadingMore ? 1 : 0),
                      itemBuilder: (_, i) {
                        ...
                        return _PostCard(
                            ...
                            onUpvote: () {
                              ...
                              ctx.read<CommunityCubit>().toggleUpvote(post.id);
                            });
                      }),
            ),
        },
        floatingActionButton: FloatingActionButton(
          ...
          onPressed: () async {
            ...
            if (newPost != null && mounted)
              ctx.read<CommunityCubit>().addPost(newPost);
          },
...
class _PostCard extends StatelessWidget {
                            ...
                            CircleAvatar(
                                ...
                                child: post.userAvatarUrl == null
                                    ? const Icon(Icons.person,
                                        size: 16, color: AppColors.dividerLight)
                                    : null),
  ...
  String _typeLabel(bool isAr) {
    switch (post.postType) {
      case 'question':
        return isAr ? 'سؤال' : 'Question';
      case 'experience':
        return isAr ? 'تجربة' : 'Experience';
      case 'announcement':
        return isAr ? 'إعلان' : 'Announcement';
      default:
        return isAr ? 'نقاش' : 'Discussion';
    }
  }
}
