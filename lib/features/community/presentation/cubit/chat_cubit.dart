...
class ChatListCubit extends Cubit<ChatListState> {
  ...
  ChatListCubit(this._r) : super(const ChatListState());
  Future<void> load() async {
    emit(state.copyWith(status: ChatStatus.loading));
    try {
      emit(state.copyWith(
          status: ChatStatus.loaded,
          conversations: await _r.getConversations()));
    } catch (e) {
...
class ChatRoomCubit extends Cubit<ChatRoomState> {
  final ChatRepository _r;
  ChatRoomCubit(this._r) : super(const ChatRoomState());
  Future<void> loadMessages(String id) async {
    ...
    try {
      ...
      _r.subscribe(id, (m) {
        if (!isClosed) emit(state.copyWith(messages: [...state.messages, m]));
      });
    } catch (e) {
